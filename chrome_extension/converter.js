/**
 * Shared X-article-to-Markdown converter.
 *
 * Works in both browser (Chrome extension) and Node.js (via jsdom).
 * In the browser it exposes a global `XConverter`.
 * In Node.js it exports via `module.exports`.
 */
const XConverter = (function () {
  ...
  function parseHeading(el) {
    const h1 = el.matches?.('h1.longform-header-one') ? el : el.querySelector('h1.longform-header-one');
    if (h1) {
      const heading = cleanText(h1.textContent || '');
    ...
    const h3 = el.matches?.('h3.longform-header-three') ? el : el.querySelector('h3.longform-header-three');
    if (h3) {
  ...
  function parseBlockQuote(el, state) {
    const lines = [];
    const directBlocks = Array.from(el.children || []).filter(
    ...
    for (const child of contentRoot.children || []) {
      ...
      if (child.tagName && (child.tagName.toLowerCase() === 'ul' || child.tagName.toLowerCase() === 'ol')) {
        ...
        for (const li of child.querySelectorAll('li')) {
          const text = cleanText(parseInlineChildren(li, state));
      ...
      if (child.querySelector && child.querySelector('h1.longform-header-one, h2.longform-header-two, h3.longform-header-three')) {
  ...
  function extractMetadata(doc, fallbackUrl) {
    const title = cleanText((doc.querySelector('[data-testid="twitter-article-title"]') || {}).textContent || '');

    const user = doc.querySelector('[data-testid="User-Name"]');
    ...
    const datetime = (doc.querySelector('time[datetime]') || {}).getAttribute?.('datetime') || '';
    const statusHrefs = Array.from(doc.querySelectorAll('a[href*="/status/"]'))
    ...
    const statusHref =
      ...
      statusHrefs[0] ||
      '';
    const articleHref = (doc.querySelector('a[href*="/article/"]') || {}).getAttribute?.('href') || '';
    const sourceUrl = resolveXUrl(statusHref || articleHref || fallbackUrl || '');

    return { title, author, handle, datetime, sourceUrl };
  }
  ...
  function extractArticleFromDoc(doc, fallbackUrl) {
    const richRoot = doc.querySelector('[data-testid="twitterArticleRichTextView"]');
    if (!richRoot) {
    ...
    const metadata = extractMetadata(doc, fallbackUrl);
  ...
  return { extractArticleFromDoc };
})();

if (typeof module !== 'undefined' && module.exports) {
  module.exports = XConverter;
}
